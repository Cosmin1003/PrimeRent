...
Tocmai aici intervine inteligența artificială ca element diferențiator. Modelele de limbaj de dimensiuni mari (LLM-urile), exemplificate de Gemini 2.5 Flash al Google, deschid posibilități concrete: generarea automată de texte descriptive, căutare bazată pe înțelegerea limbajului natural, sinteză a recenziilor. Tehnicile de embedding vectorial permit compararea semantică a textelor, depășind limitările căutării clasice prin cuvinte cheie. Aceste capabilități, integrate într-o platformă de închirieri, pot ridica semnificativ calitatea experienței pentru toți participanții.


\section{Motivația alegerii temei}

Mai mulți factori au contribuit la alegerea acestei teme.
...
\begin{enumerate}[leftmargin=2cm]
  ...
  \item \textbf{Căutare avansată} — filtre multiple (locație, interval de date, număr de oaspeți, facilități), combinate cu un modul de căutare semantică prin embedding vectorial care acceptă interogări formulate în limbaj natural.
...
\textbf{Capitolul 3} prezintă detaliat fiecare tehnologie și instrument din stack-ul ales — React, TypeScript, Vite, Tailwind CSS, Supabase, Google Gemini API, Leaflet, Stripe, Vercel — justificând selecția fiecăruia.
...
Icoanele implicite Leaflet au fost înlocuite cu icoane personalizate pentru un aspect vizual mai îngrijit și mai bine integrat în designul aplicației.
...
Funcții serverless în TypeScript/Deno, rulate la periferia rețelei \cite{supabase-edge}. Patru Edge Functions sunt definite: \texttt{generate-listing}, \texttt{smart-search}, \texttt{summarize-reviews} și \texttt{generate-embeddings}. Acestea utilizează Google Gemini API pentru toate operațiunile de inteligență artificială.

\subsection{pgvector — Extensia pentru căutare semantică}

Extensia pgvector adaugă suport pentru vectori n-dimensionali în PostgreSQL \cite{pgvector}. Fiecare proprietate are un vector de 768 dimensiuni, generat de modelul \texttt{gemini-embedding-001} al Google. La o interogare semantică, query-ul utilizatorului este convertit în același spațiu vectorial, iar similaritatea cosinus identifică proprietățile cele mai relevante.
...
\section{Inteligență Artificială — Google Gemini API}

\subsection{Gemini 2.5 Flash pentru generare și sumarizare text}

\textbf{Gemini 2.5 Flash} este un model de limbaj de mari dimensiuni (LLM) dezvoltat de Google DeepMind \cite{gemini}. Antrenat pe un corpus vast de text și date multimodale, Gemini 2.5 Flash demonstrează capacitatea de a înțelege și genera text în limbaj natural cu o calitate remarcabilă, urmând instrucțiuni complexe prin tehnica \textbf{prompt engineering} — construirea atentă a instrucțiunilor care ghidează comportamentul modelului. Modelul suportă nativ intrări multimodale (text și imagini) și generare de conținut structurat (JSON).

În PrimeRent, Gemini 2.5 Flash este utilizat în trei Edge Functions cu scopuri distincte:

\textbf{1. \texttt{generate-listing}} — Primește ca input un set de date structurate despre proprietate: titlul curent, adresa, numărul de camere, prețul, facilitățile selectate. Opțional, primește și prima imagine a proprietății codificată în base64. Generează un titlu optimizat (maxim 50 de caractere) și o descriere atractivă de două paragrafe. Suportul multimodal nativ permite modelului să menționeze detalii vizuale din fotografii — o fereastră panoramică, o grădină înflorită — care nu sunt explicit introduse în formular. Apelul se face direct prin REST la endpoint-ul \texttt{generativelanguage.googleapis.com} cu parametrul \texttt{responseMimeType: "application/json"} pentru a forța returnarea de JSON valid.

\textbf{2. \texttt{summarize-reviews}} — Primește lista textelor recenziilor unei proprietăți și generează un obiect JSON structurat cu trei câmpuri: o frază de sinteză generală, o listă de puncte forte (\textit{pros}) și o listă de aspecte negative (\textit{cons}). Parametrul \texttt{responseMimeType: "application/json"} forțează modelul să returneze exclusiv JSON valid, eliminând necesitatea de a parsa text liber și reducând riscul de erori.

\textbf{3. \texttt{smart-search}} — Utilizează modelul \texttt{gemini-2.5-flash} prin SDK-ul oficial \texttt{@google/generative-ai} (\texttt{GoogleGenerativeAI}) pentru a extrage filtre structurate din interogările în limbaj natural ale utilizatorilor.

\subsection{gemini-embedding-001 pentru căutare semantică}

\textbf{gemini-embedding-001} este un model Google specializat în generarea de \textbf{embeddings} — reprezentări vectoriale dense ale textului într-un spațiu matematic cu 768 dimensiuni \cite{gemini-embeddings}. Ideea fundamentală: texte cu înțeles similar sunt reprezentate prin vectori apropiați în acest spațiu (distanță cosinus mică), indiferent de vocabularul exact folosit. Dimensionalitatea de ieșire (768) este configurată explicit prin parametrul \texttt{outputDimensionality}, folosind SDK-ul \texttt{@google/genai} (\texttt{GoogleGenAI}).
...
\textbf{Pasul 1 — Indexare (offline).} Edge Function \texttt{generate-embeddings} procesează proprietățile din baza de date în batch-uri de 50. Pentru fiecare, concatenează titlul, orașul și descrierea, generează embedding-ul prin API-ul Google Gemini și îl stochează în coloana \texttt{embedding} (de tip \texttt{vector(768)}) din tabela \texttt{properties}. Acest pas se rulează periodic din panoul de administrare.
...
\textbf{CF-G12 — Gestionarea profilului personal.} Pagina de setări permite utilizatorului să își personalizeze prezența pe platformă: editarea numelui complet, adăugarea unui număr de telefon valid și a datei nașterii. Aceste date sunt absolut necesare pentru a se putea transforma ulterior din oaspete simplu în gazdă.
...
Componentele arhitecturale: browserul utilizatorului (React SPA), Vercel CDN (fișiere statice), Supabase Cloud (PostgREST API, PostgreSQL cu pgvector, Storage, Edge Functions), Google Gemini API (Gemini 2.5 Flash și gemini-embedding-001) și Stripe API (procesare plăți).
...
Butonul ,,AI Search'' invocă Edge Function \texttt{smart-search}, care procesează query-ul utilizatorului în trei etape. Prima: convertirea textului în embedding vectorial cu \texttt{gemini-embedding-001}. A doua: compararea vectorului rezultat cu toți vectorii proprietăților din baza de date, prin funcția PostgreSQL \texttt{match\_properties}. A treia, opțională: extragerea de filtre structurate din textul query-ului (locație, număr de oaspeți) folosind \texttt{gemini-2.5-flash}, și aplicarea lor automată în interfață.
...
\begin{enumerate}
  ...
  \item \textbf{Facilitățile} — prin join pe \texttt{property\_amenities} și \texttt{amenities}.
...
Funcționalitatea ,,Generate with AI'' colectează toate datele din formular (titlu, adresă, facilități, preț, capacitate) și prima imagine (codificată base64). Acestea sunt trimise la Edge Function \texttt{generate-listing}, care construiește un prompt structurat pentru Gemini 2.5 Flash. Când există o imagine, mesajul devine multimodal — modelul analizează fotografia prin câmpul \texttt{inlineData} și poate menționa elemente vizuale (o priveliște montană, o terasă, un interior luminos) în descrierea generată.
...
Din partea oaspetelui, \texttt{BookingsPage} afișează rezervările cu etichete colorate în funcție de status. Anularea actualizează starea local fără refetch complet (actualizare optimistă).
...
La finalizarea cu succes a unei rezervări, platforma trimite automat oaspetelui un email de confirmare cu detaliile sejurului.
...
Deno.serve(async (req) => {
  ...
  // 3. Apel API extern (Google Gemini, Stripe)
...
Funcția construiește un prompt structurat din datele proprietății: locație, capacitate, facilități, preț. Când gazda a uploadat o imagine, mesajul către Gemini 2.5 Flash devine \textbf{multimodal} — include atât text cât și imagine codificată base64 (câmpul \texttt{inlineData}), permițând modelului să observe și să descrie detalii vizuale din fotografie.

Parametrul \texttt{responseMimeType: "application/json"} forțează Gemini să returneze exclusiv JSON valid cu structura predefinită (\texttt{title} și \texttt{description}), eliminând necesitatea de parsare a textului liber și reducând riscul de erori.

\subsection{Căutare semantică (\texttt{smart-search})}

Cea mai complexă Edge Function, cu trei pași secvențiali. \textbf{Primul}: conversia query-ului utilizatorului în vector de embedding prin \texttt{gemini-embedding-001} (768 dimensiuni). \textbf{Al doilea}: invocarea funcției PostgreSQL \texttt{match\_properties}, care compară vectorul cu toți vectorii din baza de date folosind operatorul \texttt{<=>} (distanță cosinus). Sunt returnate cele mai relevante proprietăți cu scor de similaritate peste pragul de 0.75.

\textbf{Al treilea} pas, opțional dar foarte util, folosește \texttt{gemini-2.5-flash} (prin SDK-ul \texttt{@google/generative-ai}) pentru a extrage filtre structurate din textul query-ului. Dacă utilizatorul scrie ,,apartment in Cluj for 3 guests'', modelul extrage \texttt{\{city: "Cluj", guests: 3\}} și le aplică automat în interfață.

\subsection{Sumarizare recenzii (\texttt{summarize-reviews})}

Primește textele tuturor recenziilor unei proprietăți, concatenate într-un singur string. Prompt-ul instruiește \texttt{gemini-2.5-flash} să returneze un obiect JSON cu trei câmpuri: un rezumat narativ general, o listă de puncte forte (\texttt{pros}) și o listă de aspecte negative (\texttt{cons}). Parametrul \texttt{responseMimeType: "application/json"} garantează structura răspunsului. Gazda și potențialii oaspeți obțin astfel o sinteză rapidă, fără a fi nevoiți să citească zeci de recenzii individual.
...
Comutarea manuală se face prin componenta \texttt{LanguageSwitcher} din navbar, care apelează \texttt{i18n.changeLanguage()} și re-randează aplicația. Toate textele vizibile sunt externalizate în fișierele \texttt{ro.json} și \texttt{en.json}, organizate pe domenii: \texttt{navbar}, \texttt{home}, \texttt{bookings}, \texttt{explore}, etc.
...
Unitar & \texttt{getAmenityDetails} — mapare facilități & 7 \\
...
\textbf{Maparea facilităților} — 7 teste: verificarea facilităților specifice (Wifi returnează iconița Wifi), valoarea fallback pentru o facilitate necunoscută (returnează icon-ul generic \texttt{ShieldCheck}), și un test parametrizat care iterează prin toate cele 20 de facilități definite.
...
Aplicația necesită două variabile prefixate \texttt{VITE\_}: URL-ul și cheia anonimă Supabase. Cheia anonimă este publică prin design — securitatea este asigurată de RLS. Cheile secrete (\texttt{GEMINI\_API\_KEY}, Stripe) sunt stocate exclusiv ca Supabase Secrets, accesibile doar din Edge Functions.
...
\begin{itemize}
    \item \textbf{Destinații Populare (Popular Destinations):} Un carusel curat, care prezintă cele mai căutate orașe din baza de date, însoțite de fotografii atrăgătoare și o etichetă ce semnalizează câte proprietăți sunt listate momentan în acel oraș (ex: „Cluj-Napoca — 12 properties'').
...
Partea superioară este dominată permanent de \textbf{Bara de navigare (Navbar)}. Aceasta are un design adaptiv și inteligent. Pe un ecran mare, ea afișează toate butoanele aliniate orizontal; pe telefonul mobil, acestea se retrag curat sub un singur buton (meniul de tip „hamburger'') pentru a nu ascunde ecranul principal. 
Bara va afișa informații diferite în funcție de cine se uită la ea: dacă nu ești autentificat, vezi doar opțiunile „Explore'' și „Login''; dacă ești oaspete, vor apărea „Favorites'' și „My Bookings''; iar dacă ești gazdă înregistrată, va fi vizibilă opțiunea „Dashboard''.
...
\begin{itemize}
    \item \textbf{Autentificarea tradițională (Email și Parolă):} Utilizatorul introduce o adresă validă de email și setează o parolă. Ulterior, trebuie să-și acceseze căsuța de email și să confirme adresa accesând linkul unic primit de la Supabase, obținând abia apoi acces pe platformă.
...
În partea superioară stă un bloc solid cu câmpuri clare: Destinație, Calendar (Selectarea datei plecării și a sosirii prin două click-uri consecutive) și Număr de oaspeți. Dacă se apasă butonul \textbf{„Filters''}, un dialog lateral (Sheet) culisează și expune o grilă cu toate cele 20 de facilități posibile (WiFi, Parcare, etc.). Utilizatorul bifează ce dorește și apasă „Apply''. Din acel moment, serverul filtrează masiv și expune un grid de carduri elegante doar cu proprietățile disponibile \textit{strict în acea perioadă de timp}. Proprietățile indisponibile nici măcar nu vor fi returnate pentru a nu aglomera vizual. Grid-ul poate fi sortat rapid folosind meniul vertical „Sort By'' din colțul dreapta.
...
Fiecare card din rezultate are o \textbf{inimioară} în colț. Printr-un simplu click, proprietatea este salvată în secțiunea \textbf{„Favorites''} pentru a fi revizitată ulterior, facilitând astfel procesul de explorare preliminară a opțiunilor de vacanță. Pe dispozitivele mobile, utilizatorul are la dispoziție un buton fix în colțul din stânga jos pentru a comuta rapid între lista de proprietăți și o hartă interactivă pe tot ecranul, ce poate fi navigată fluid prin gesturi tactile.

\section{Fluxul Oaspetelui: Pagina Proprietății, Generarea Rezumatului AI și Plata}

Făcând click pe o proprietate din căutare, oaspetele deschide o vizualizare individuală detaliată (ruta \texttt{/explore/:id}). Ecranul este împărțit în două pe ecranele cu rezoluție mare.

La stânga, conținutul: o galerie asimetrică ce poate fi deschisă în ecran complet apăsând orice poză, date tehnice detaliate și lista completă de facilități disponibile sau tăiate (indisponibile). Imediat sub ele se află harta clădirii și lista de recenzii anterioare.
\textbf{Cum se folosește rezumarea AI a recenziilor:} Imediat deasupra comentariilor este afișat un card albastru intitulat \textit{AI Review Summary}. La apăsarea butonului din interior, utilizatorul trebuie să aștepte în medie 5-8 secunde, timp în care aplicația apelează direct modelul Google Gemini 2.5 Flash. Când procesul se termină, cardul se extinde și livrează un raport clar, punând în evidență dacă locuința are sau nu probleme recurente (ex: „Mulți oaspeți s-au plâns de zgomotul de la stradă, dar laudă priveliștea'').

În partea dreaptă, panoul interactiv de rezervare rămâne fixat pe ecran în timpul derulării paginii.
...
6. Imediat după confirmarea rezervării, oaspetele primește automat un \textbf{email de confirmare} cu detaliile complete ale sejurului: proprietatea rezervată, intervalul de date și prețul total plătit.
...
Utilizatorul adaugă pozele de pe propriul disc; platforma nu le urcă direct online, ci generează miniaturi de previzualizare locale, astfel încât dacă o imagine este selectată greșit, gazda o poate șterge printr-un simplu click pe iconița de sub ea, fără a genera trafic de rețea inutil.

\textbf{Secretul gazdei pentru atragerea oaspeților:} În loc să redacteze manual texte de promovare, gazda trebuie doar să completeze informațiile factuale de bază, să încarce prima poză și să lase secțiunile Titlu și Descriere goale. Apăsarea butonului \textbf{„Generate with AI''} va citi tot formularul plus fotografia și, în câteva secunde, va autocompleta câmpurile de titlu și descriere cu texte atractive, generate la nivel profesional. 

Gazda apasă Publish, iar fotografiile sunt încărcate printr-o conexiune securizată în sistemul Supabase Storage, devenind imediat vizibile public pentru toți oaspeții, fără a necesita aprobarea manuală a unui administrator.

Orice gafă se repară din „Manage Listings'', unde gazda are la dispoziție un tabel cu absolut toate casele proprii publicate, alături de un mic creion de Edit. Apăsarea sa deschide fix formularul anterior, precompletat, pentru un proces lin de schimbare a pozelor sau actualizare de preț.

\section{Modificarea setărilor regionale: Limba platformei}

PrimeRent este complet internaționalizată. În bara superioară, componenta \texttt{LanguageSwitcher} (marcată cu \textbf{RO} / \textbf{EN}) reprezintă interfața utilizatorului cu motorul i18next. 
Acționarea acestui buton va actualiza instantaneu toate textele din site. De exemplu, butonul „Explore Destinations'' va deveni „Explorează destinații''. Această preferință este salvată în memoria browser-ului (în \texttt{localStorage}), astfel încât la următoarele accesări ale platformei PrimeRent, interfața va fi încărcată automat în limba aleasă anterior.
...
Ca produs MVP, PrimeRent are și limite: nu oferă mesagerie directă gazdă-oaspete, anulările nu declanșează rambursări automate prin Stripe, nu acoperă teste E2E, imaginile nu sunt optimizate la upload, prețurile sunt afișate exclusiv în USD.
...
Integrarea AI a demonstrat că funcționalitățile de inteligență artificială nu mai sunt exclusivitatea echipelor mari. Prin Google Gemini API și Edge Functions Supabase, un singur dezvoltator poate integra capabilități de nivel enterprise.
...
Gemini Team Google, \textit{Gemini: A Family of Highly Capable Multimodal Models}, arXiv:2312.11805, 2023.
...
\bibitem{gemini}
Google DeepMind, \textit{Gemini API Documentation}, 2024. [Online]. Disponibil la: \url{https://ai.google.dev/gemini-api/docs}

\bibitem{gemini-embeddings}
Google DeepMind, \textit{Gemini Embedding Models — gemini-embedding-001}, 2024. [Online]. Disponibil la: \url{https://ai.google.dev/gemini-api/docs/embeddings}

\bibitem{gemini-vision}
Google DeepMind, \textit{Gemini Multimodal Inputs — Vision capabilities}, 2024. [Online]. Disponibil la: \url{https://ai.google.dev/gemini-api/docs/vision}
